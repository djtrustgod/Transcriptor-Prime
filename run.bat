...
rem  (~170 MB download). Every run after that starts the app directly.
rem ---------------------------------------------------------------------
setlocal
cd /d "%~dp0"
...
if not exist "%PYTHON%" (
    ...
    echo This downloads about 170 MB and takes a few minutes.
    echo.
...
"%PYTHON%" -c "import customtkinter, faster_whisper, av, sherpa_onnx" >nul 2>nul
if errorlevel 1 (
